...
def user(db, django_user_model):
    ...
    user = django_user_model.objects.create_user(name='Test', surname='123', email='test@test.pl',
    ...
    user.user_permissions.add(permission_add_club, permission_add_court, permission_add_couch, permission_edit_club,
                              permission_edit_coach, permission_delete_club, permission_delete_coach,
...
def user_no_perm(db, django_user_model):
    user = django_user_model.objects.create_user(email='test1@admin.com', name='Test1 User', surname='Test2',
...
def create_club(user):
    yield Club.objects.create(name='Test', location='Test1', quantity=1, multisport=1, user=user)
...
def create_coach(user, create_club):
    yield Coach.objects.create(name='Test2', surname='Test1', price=50, club_id=create_club.id)

@pytest.fixture()
def create_court(user, create_club):
    yield Club.objects.create(name='Test3', type=1, preference=1, club_id=create_club.id)
